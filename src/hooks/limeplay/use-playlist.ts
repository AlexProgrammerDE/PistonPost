    ...
    createSlice: (set, get, _store, events) => ({
      [PLAYLIST_FEATURE_KEY]: {
        ...
        cycleRepeatMode: () => {
          ...
          const currentMode = normalizeRepeatMode(playlist.repeatMode, playlist.queue.length)
        ...
        getPreviousIndex: () => {
          ...
          if (playlist.history.length > 0) {
            ...
            const idx = playlist.queue.findIndex((entry) => entry.id === lastItem.id)
            if (idx !== -1) return idx
          }
          ...
          return playlist.currentIndex > 0 ? playlist.currentIndex - 1 : playlist.currentIndex
        },
        history: [],
        insert: (items, atIndex) => {
          ...
          if (playlist.shuffle) {
            ...
            const newIndices = Array.from({ length: normalized.length }, (_, i) => idx + i)
        ...
        load: (items, startIndex = 0) => {
          const playlist = get().playlist as PlaylistStore["playlist"]
          const normalized = normalizeItems(items)
          const normalizedStartIndex = clamp(startIndex, 0, Math.max(normalized.length - 1, 0))
          ...
          if (normalized.length > 0) {
            ...
            emitPlaylistChange(events.emit, normalizedStartIndex, startItem, -1, null, "load")
          }
        },
        newSession: () => {
        ...
        prepend: (items) => {
          ...
          let newShuffleOrder = playlist.shuffleOrder.map((idx) => idx + normalized.length)

          if (playlist.shuffle) {
            const newIndices = Array.from({ length: normalized.length }, (_, i) => i)
        ...
        remove: (id) => {
          ...
          if (removeIndex < playlist.currentIndex) {
            ...
            newCurrentIndex = Math.min(playlist.currentIndex, newQueue.length - 1)
          ...
          set(({ playlist: p }) => {
            ...
            p.shuffleOrder =
              enabled && playlist.queue.length > 0
                ? createShuffleOrder(playlist.queue.length, playlist.currentIndex)
                : []
          })
        },
        shuffle: false,
...
export function usePlaylist<T>(): UsePlaylistReturn<T> {
  ...
  const hasNext =
    (repeatMode === "all" && queue.length > 0) || api.getState().playlist.getNextIndex() !== -1

  const hasPrevious =
    (repeatMode === "all" && queue.length > 0) || api.getState().playlist.getPreviousIndex() !== -1
...
export function usePlaylistStore<TSelected>(
  ...
  return useMediaFeatureStore<PlaylistStore, TSelected>(PLAYLIST_FEATURE_KEY, (state) =>
    selector(state.playlist),
  )
}
...
function normalizeRepeatMode(mode: RepeatMode, queueLength: number): RepeatMode {
