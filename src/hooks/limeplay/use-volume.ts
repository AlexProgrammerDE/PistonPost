...
export function useVolumeStore<TSelected>(
  ...
  return useMediaFeatureStore<VolumeStore, TSelected>(VOLUME_FEATURE_KEY, (state) =>
...
function VolumeSetup() {
  ...
  React.useEffect(() => {
    ...
    on(media, ["loadstart", "loadedmetadata", "loadeddata", "emptied"], audioTracksChangedHandler)
