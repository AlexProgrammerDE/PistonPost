...
import { type PlayerStore, usePlayerStore } from "@/hooks/limeplay/use-player"
      ...
      [TIMELINE_FEATURE_KEY]: {
        ...
        seek: (time) => {
          ...
          set(({ timeline }) => {
            ...
            timeline.progress = timeline.duration > 0 ? storeCurrentTime / timeline.duration : 0
          })
...
export function useTimelineStore<TSelected>(
  ...
  return useMediaFeatureStore<TimelineStore, TSelected>(TIMELINE_FEATURE_KEY, (state) =>
...
function TimelineSetup() {
  ...
  const onTimeUpdate = React.useCallback(() => {
    ...
    if (isLive) {
      ...
      liveLatency = mediaElement.currentTime === 0 ? 0 : seekRange.end - mediaElement.currentTime

      liveLatency = toFixedNumber(clamp(liveLatency, 0, seekRangeSize), 4)
      currentTime = clamp(mediaElement.currentTime - seekRange.start, 0, seekRangeSize)
      progress =
        seekRangeSize > 0 ? 1 - (seekRange.end - mediaElement.currentTime) / seekRangeSize : 0

      progress = toFixedNumber(clamp(progress, 0, 1), 4)
    } else {
      currentTime = clamp(mediaElement.currentTime, 0, timeline.duration)
      progress = timeline.duration > 0 ? toFixedNumber(currentTime / timeline.duration, 4) : 0
    }
  ...
  const onDurationChange = React.useCallback(() => {
    ...
    const seekRange = player.seekRange()
    const playerDuration = player.isLive() ? seekRange.end - seekRange.start : mediaElement.duration
  ...
  }, [canPlay, mediaElement, onBuffer, onDurationChange, onLoading, onTimeUpdate, player])
