...
const SHAKA_DOCS_BASE = "https://shaka-player-demo.appspot.com/docs/api/shaka.util.Error.html"
  ...
  ({ children, className, description, error, message, ...props }, ref) => {
    ...
    return (
      <div
        ...
        {...props}
      >
        <Item className={`max-w-md px-4 sm:px-8`}>
          <ItemContent className={`gap-1.5 sm:gap-2`}>
            <ItemTitle className={`text-sm font-semibold sm:text-base`}>
              {message ?? details.title}
            </ItemTitle>
            <ItemDescription className={`text-xs font-medium text-muted-foreground sm:text-sm`}>
              Error code: {details.code}
            </ItemDescription>
            <ItemDescription className={`text-xs/relaxed text-muted-foreground/60 sm:text-sm`}>
...
export function getErrorDetails(error: unknown): ErrorDetails {
  ...
  if (isShakaError(error)) {
    const categoryLabel = SHAKA_CATEGORY_MAP[error.category] ?? `Category ${error.category}`
...
export function isMediaError(error: unknown): error is MediaErrorLike {
  ...
  return !hasNumberProperty(error, "category") && hasNumberProperty(error, "code")
}
