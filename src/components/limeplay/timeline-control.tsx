...
>((props, ref) => {
  ...
  return (
    <SliderPrimitive.Root
      ...
      className={cn(
        `relative block data-[orientation=horizontal]:h-(--lp-timeline-track-height)`,
        className,
      )}
...
>((props, ref) => {
  ...
  return (
    <SliderPrimitive.Track
      className={cn(
        `relative flex h-full grow flex-row bg-foreground/20 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary/50`,
        className,
      )}
      ref={ref}
      tabIndex={0}
...
export const Buffered = React.forwardRef<HTMLDivElement, BufferedProps>((props, ref) => {
  ...
  const duration = useTimelineStore((state) => state.duration)
  const processBufferedRanges = useTimelineStore((state) => state.processBufferedRanges)
...
export const Thumb = React.forwardRef<HTMLDivElement, ThumbProps>((props, ref) => {
  ...
  return (
    <SliderPrimitive.Thumb
      className={cn(
        `left-(--lp-timeline-thumb-position)! bg-primary data-disabled:bg-primary/85`,
        className,
      )}
      ref={ref}
      {...etc}
