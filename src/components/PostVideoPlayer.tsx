...
const LimeplayVideoPlayer = lazy(() =>
  import("@/components/video-player/player").then(({ VideoPlayer }) => ({
    default: VideoPlayer,
  })),
)
    ...
    <div ref={containerRef} className="size-full">
      {shouldLoadPlayer ? (
        <Suspense fallback={poster}>
          <LimeplayVideoPlayer
            source={`/media/video/${mediaId}/manifest`}
            sourceKey={mediaId}
            layout="fill"
            theme="dark"
            className="size-full"
            mediaProps={{
              ...
              playsInline: true,
              poster: `/media/video/${mediaId}/thumbnail?v=${VIDEO_THUMBNAIL_CACHE_VERSION.toString()}`,
