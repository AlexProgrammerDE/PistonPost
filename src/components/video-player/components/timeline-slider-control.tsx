...
export function TimelineSliderControl() {
  ...
  return (
    <div className="me-auto flex grow items-center gap-1.5 select-none @3xl/root:gap-3">
      {!isLive && <Elapsed className="px-2 text-xs font-medium" />}
      <div className="group/timeline relative w-full grow">
        <TimelineSlider.Root
          className={`group hit-area-x-[2px] hit-area-y-[14px] hit-area h-1 cursor-crosshair rounded-full transition-[height] duration-150 ease-in-out active:data-[orientation=horizontal]:h-(--lp-timeline-track-height-active)`}
        >
          ...
          <TimelineSlider.Thumb
            className={`absolute h-8 w-px rounded-full bg-primary/60 opacity-0 transition-opacity duration-300 group-hover/timeline:opacity-100 group-active/timeline:bg-white`}
            showWithHover
          />
          <TimelineSlider.Thumb
            className={`top-auto! bottom-[calc(100%+16px)] flex h-auto w-fit bg-transparent text-xs font-medium opacity-0 transition-opacity duration-300 group-hover/timeline:opacity-100`}
            showWithHover
          >
            {isLive && <>&minus;</>}
            <HoverTime />
      ...
      {!isLive && (
        <button
          ...
          className="inline-flex h-7 w-[6ch] cursor-pointer items-center justify-center rounded-sm focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary/50 focus-visible:outline-solid"
          onClick={() => {
          ...
          {showGoToLive && player && (
            <>
              ...
              <Button
                ...
                className="cursor-pointer px-2 @md/root:px-2.5 @3xl/root:px-3"
