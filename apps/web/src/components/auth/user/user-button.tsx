...
import { ChevronsUpDown, LogIn, LogOut, Settings, UserPlus2 } from "@/components/icons"
...
function renderUserLink(
  ...
  return (
    <DropdownMenuItem key={href} variant={variant} onClick={() => navigate({ to: href })}>
      {icon}
      {label}
    </DropdownMenuItem>
  )
      ...
      >
        ...
        {session ? (
          <>
            ...
            {plugins.flatMap((plugin) =>
              plugin.userMenuItems?.map((Item) => (
                <Item key={componentIdentity(plugin.id, "signed-in-menu", Item)} />
              )),
            )}
          ...
          <>
            ...
            {plugins.flatMap((plugin) =>
              plugin.userMenuItems?.map((Item) => (
                <Item key={componentIdentity(plugin.id, "signed-out-menu", Item)} />
              )),
            )}
          </>
        )}
