...
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@pistonpost/ui/components/tabs"
import { cn } from "@pistonpost/ui/lib/utils"
import { useMemo } from "react"

import { Shield, User2 } from "@/components/icons"
  ...
  return (
    <Tabs value={currentView} className={cn("w-full gap-4 md:gap-6", className)}>
          ...
          {plugins.flatMap(
            (plugin) =>
              plugin.settingsTabs?.map((settingsTab) => (
                <TabsTrigger
                  key={`${plugin.id}:settings-tab:${settingsTab.view}`}
      ...
      {plugins.flatMap((plugin) =>
        plugin.settingsTabs?.map((settingsTab) => (
          <TabsContent
            key={componentIdentity(plugin.id, settingsTab.view, settingsTab.component)}
            value={settingsTab.view}
            tabIndex={-1}
          >
