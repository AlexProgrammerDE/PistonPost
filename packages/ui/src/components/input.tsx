...
function Input({ className, type, ...props }: React.ComponentProps<"input">) {
  const { onChange, onInput, ...inputProps } = props
  return (
    <InputPrimitive
      type={type}
      data-slot="input"
      ...
      onInput={(event) => {
        onInput?.(event)
        // React delivers the same synthetic input event shape to both handlers. This adapter keeps
        // the native shadcn onChange contract while Base UI retains its own value bookkeeping.
