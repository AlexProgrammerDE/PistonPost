...
export default function Layout({children}: { children: ReactNode }) {
  ...
  const {
    data: healthData,
    error: healthError
  } = useSWR<HealthResponse>('/api/health', url => fetch(url).then(res => res.json()), {refreshInterval: 5000});
  const {status} = useSession();
  const {data: settingsData} = useSWR<AccountSettings>(status === "authenticated" ? '/settings' : null, {refreshInterval: 5000});
