...
const VoteRow = ({
  ...
  return (
    <div className="flex flex-row">
      <button
        onClick={() => vote("LIKE", likes.voted)}
        className={"btn ml-1 mr-0.5" + (likes.voted ? " btn-primary" : "")}
      >
        👍️️ {String(likes.value)}
      </button>
      <button
        onClick={() => vote("DISLIKE", dislikes.voted)}
        className={"btn mx-0.5" + (dislikes.voted ? " btn-primary" : "")}
      >
        👎 {String(dislikes.value)}
      </button>
      <button
        onClick={() => vote("HEART", hearts.voted)}
        className={"btn mx-0.5" + (hearts.voted ? " btn-primary" : "")}
      >
        ♥️ {String(hearts.value)}
      </button>
    </div>
  );
};
    ...
    <Layout>
      ...
      <div className="break-text container flex-grow p-2">
          ...
          <div className="ml-1 flex flex-wrap gap-1">
            {postData.tags.map((tag) => (
              <PostOptionColumn key={tag}>
                <Link href={"/tag/" + tag} className="badge badge-lg">
                  #{tag}
                </Link>
              </PostOptionColumn>
            ))}
            ...
            <PostOptionColumn>
              <label htmlFor="copy-modal" className="modal-button">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 24 24"
                  fill="currentColor"
        ...
        <div className="rounded-box mt-2 flex w-full flex-wrap bg-base-200 p-4 p-4">
          <Link
            href={`/user/${postData.authorData.name}`}
        ...
        <label htmlFor="copy-modal" className="modal cursor-pointer">
          <label className="modal-box relative" htmlFor="">
            <h3 className="mb-2 text-lg font-bold">Chose what to copy!</h3>
            <div className="flex flex-col gap-2">
              ...
              {postData.type === "IMAGES" && (
                <div>
                  <CopyButton
                    text={postData
                      .images!.slice(0, Math.min(5, postData.images!.length))
                      .map((_, index) => `${postUrl}?image=${index}`)
                      .join("\n")}
                  >
                    <button className="btn whitespace-nowrap">
                      <a>Copy Image Links</a>
                    </button>
                  </CopyButton>
                </div>
              )}
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  ...
  context.req.rawHeaders.forEach((header) => {
    if (i % 2 === 0) {
      reqHeaders.append(
        context.req.rawHeaders[i],
        context.req.rawHeaders[i + 1]
      );
    }
    i++;
  });
