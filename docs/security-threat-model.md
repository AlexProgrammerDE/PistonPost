...
This threat model covers the initial Cloudflare deployment. It treats D1, private R2, Images, Stream, Queues, Workflows, Email Service, and Secrets Store as trusted platform boundaries. Browsers, uploaded files, legacy backups, webhook requests, queue bodies, and all user-controlled text are untrusted.
...
- Migration credentials, source backups, mapping state, and discrepancy reports.
...
| ---------------------- | --------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------- |
| Authentication         | Credential stuffing, CAPTCHA bypass, migrated-account claim, stale privilege            | Turnstile, auth rate limit, normalized email claim, explicit account-linking rules, fresh database role checks                                                   | Better Auth tests, request-security tests, admin role regression test |
...
| Queue and Workflow     | Duplicate work, poison messages, private data in logs                                   | Idempotency keys, outbox claims, bounded retries, dead-letter archive with metadata only, resumable Workflow steps                                               | queue schema and deletion tests                                       |
| Migration              | Credential leakage, target overwrite, duplicate imports, missing media, source mutation | Read-only source, deterministic IDs, target collision refusal, fingerprinted runs, immutable checksum keys, quarantine reports, explicit production confirmation | migration fixture rehearsal and reports                               |
...
- A real preview migration and production penetration review still require provisioned infrastructure and operator credentials.
