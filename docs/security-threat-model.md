...
| Image upload           | IDOR, spoofed MIME, hidden private metadata, polyglot or oversized image, decompression bomb, quota abuse | Owner-bound intent, client-side Rust sanitizer, server-side sanitizer idempotence gate, short expiry, exact size, Images metadata sniffing, extension and MIME agreement, 80 MP limit, 15 MB limit, per-user rate limit | Rust sanitizer tests, upload route checks, and media state tests      |
